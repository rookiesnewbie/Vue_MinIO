...
<script>
import popper from '@/components/timeLine/popper'
const format = n => {
  return n > 9 ? n : `0${n}`
}
...
export default {
  ...
  methods: {
    formatTime(num) {
      const time = String(num).split('.')
      const hour = format(time[0])
      const minute = time[1] ? format(Number(`0.${time[1]}`) * 60) : '00'
      return `${hour}:${minute}`
    },
    formatTimeList(list) {
      const [min, max] = list
      const minutes = ((max - min) / this.scale) * this.unit
      return `${this.formatTime(min)}~${this.formatTime(max)} ${minutes}分钟`
    },
    getnum(num) {
      const time = String(num).split('.')
      const minute = time[1] ? format(Number(`0.${time[1]}`) * 60) : '0'
      return minute
    },
    getHoursList() {
      const nowTime = getTimestamp(new Date())
      const hour = `${nowTime.ymd} ${nowTime.hm}`
      const arr = []
    ...
    SecondsClick(hour, index, e) {
      /** 如果选择的区间包括禁用了的区间**/
      const key = this.hourList.findIndex((u, ukey) => {
    ...
    ClickVslider(hour, index, e) {
      ...
      const data = this.hourList.find(i => i.hour === hour)
      const showdata = this.hourList.find(i => i.visible) // 点击禁用按钮不触发。
  ...
  mounted() {
    this.$refs[this.domRef].addEventListener('mousemove', e => {
      if (this.isMove) {
        ...
        const index = e.target.getAttribute('hourkey')
        const key = this.selectHours.length
        ...
        const [min, max] = key > index ? [index, key] : [key, index]
        const uList = this.hourList.slice(min, Number(max) + 1)
        if (uList.find(i => i.overdue || i.disabled || i.checked)) {
          return
        }
