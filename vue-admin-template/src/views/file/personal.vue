  ...
  <div class="app-container">
    <el-row :gutter="20">
      <el-row :gutter="10" class="mb8" style="margin-bottom:10px">
          <el-col :span="1.5">
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleCreateFolder">新增文件夹</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="primary" icon="el-icon-upload2" size="mini" @click="handleUploadFile">上传文件</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="primary" icon="el-icon-download" size="mini" @click="handleBatchDownload" :loading="exportLoading">批量下载</el-button>
          </el-col>
          <el-col :span="6">
        ...
        <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button size="mini" type="text" icon="el-icon-download" @click="handleDownload(scope.row)"
              v-if="scope.row.type !== '文件夹'">下载</el-button>
            <el-dropdown @command="(command) => handleCommand(command, scope.$index, scope.row)">
              <span class="el-dropdown-link">
                <i class="el-icon-d-arrow-right el-icon--right"></i>更多
              </span>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="handleRename" size="mini" type="text" icon="el-icon-edit-outline">重命
                  名</el-dropdown-item>
    ...
    <!-- 新建文件夹 或者重命名 对话框 -->
    <el-dialog :title=" form.title " :visible.sync=" open " :close-on-click-modal=" false " width="550px"
      @close=" handleDialogClose ">
      <el-form class="add-folder-form" :model=" form " :rules=" formRules " ref="addOrRenameFileForm" label-width="100px"
        label-position="top">
        <el-form-item v-if=" form.isShare === false " :label=" form.label " prop="filename">
          <el-input v-model=" form.filename " :placeholder=" form.placeholder " type="textarea" autosize maxlength="255"
            show-word-limit></el-input>
        </el-form-item>
        <el-form-item v-else label="文件共享至" prop="mountFolderId">
          <treeselect v-model=" form.mountFolderId " :options=" shareFolderOptions " :show-count=" true " placeholder="请选择共享位置"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading=" sureBtnLoading " @click=" handleRenameDialogSure " v-else>确 定</el-button>
      </div>
    </el-dialog>
    ...
    <!-- 预览文件弹出框 -->
    <el-dialog :title=" showFile.name " :visible.sync=" dialogVisible " width="50%" height="100%">
...
export default {
  ...
  methods: {
      ...
      this.$refs['addOrRenameFileForm'].validate((valid) => {
        if (valid) {
          shareFileTo({
            ...
            this.$message.success('文件共享操作成功')
            this.$refs['addOrRenameFileForm'].resetFields()
            this.open = false
            this.handleQuery()
          }).catch(res => {
            this.sureBtnLoading = false
            this.$message.error('文件共享操作失败')
          })
        } else {
          this.sureBtnLoading = false
          return false
      ...
      this.$refs['addOrRenameFileForm'].validate((valid) => {
        if (valid) {
          renameFile({
            ...
            this.sureBtnLoading = false
            this.$message.error(this.form.title + '失败')
          })
        } else {
          this.sureBtnLoading = false
          return false
    ...
    // 删除文件 / 文件夹
    handleDelete(row) {
      this.$confirm('是否确认删除文件(夹)“' + row.name + '"?').then(function () {
        return removeFileToRecycleBin(row.id)
      }).then(() => {
        this.$message.success('文件删除成功，七天内可在回收站进行恢复')
        this.handleQuery()
      }).catch(error => {
        if (error === 'cancel') {
